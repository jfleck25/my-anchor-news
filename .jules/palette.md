...
## 2026-04-19 - Modal Backdrop Dismissal UX
...
**Action:** Always implement `onClick={onClose}` on the outer backdrop container and `onClick={(e) => e.stopPropagation()}` on the inner content container to support implicit dismissal without interfering with internal click interactions.
