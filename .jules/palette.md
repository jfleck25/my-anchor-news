...
## 2025-03-01 - Implicit dismiss interactions for Modals
**Learning:** Modals that do not close when a user clicks outside the modal content are frustrating and lack typical UX patterns. Implementing implicit dismiss interactions allows the modal to close intuitively.
**Action:** Always provide an implicit dismiss interaction for modals by adding an `onClick` handler to the outer backdrop to close the modal, and `onClick={(e) => e.stopPropagation()}` to the inner modal content container so clicking inside the modal does not inadvertently close it.
