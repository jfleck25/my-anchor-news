...
def generate_audio():
    ...
    try:
        ...
        # ⚡ Bolt: Use string builder pattern and track byte length to avoid O(N^2) memory overhead
        # and redundant O(N) UTF-8 encoding recalculations in the loop.
        current_chunk_parts = []
        current_chunk_byte_len = 0
        byte_limit = 4800

        for sentence in sentences:
            sentence_bytes_len = len(sentence.encode('utf-8'))
            if current_chunk_byte_len + sentence_bytes_len + 1 < byte_limit:
                current_chunk_parts.append(sentence)
                current_chunk_parts.append(" ")
                current_chunk_byte_len += sentence_bytes_len + 1
            else:
